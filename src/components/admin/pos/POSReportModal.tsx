    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
                ...
                className={`border rounded-3xl w-full max-w-4xl overflow-hidden shadow-2xl transition-all ${isLightTheme ? 'bg-white border-emerald-100' : 'bg-zinc-900 border border-zinc-800'}`}
            >
                <div className="p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-3">
                                <div className={`border col-span-2 sm:col-span-1 rounded-2xl p-4 transition-all ${isLightTheme ? 'bg-emerald-50 border-emerald-100' : 'bg-green-500/10 border-green-500/20'}`}>
                                    <p className={`text-[10px] uppercase font-black tracking-widest mb-1 ${isLightTheme ? 'text-emerald-600' : 'text-green-400'}`}>Caisses Totales</p>
                                    <p className={`text-3xl font-black leading-none ${isLightTheme ? 'text-emerald-950' : 'text-white'}`}>{reportData.totalSales.toFixed(2)} €</p>
                                </div>
                                <div className={`rounded-2xl p-4 border flex flex-col justify-center transition-all ${isLightTheme ? 'bg-white border-emerald-100' : 'bg-zinc-800/50 border-zinc-800'}`}>
                                    <p className={`text-[10px] uppercase font-bold tracking-wider mb-1 ${isLightTheme ? 'text-emerald-600/40' : 'text-zinc-500'}`}>Commandes</p>
                                    <p className={`text-2xl font-black leading-none ${isLightTheme ? 'text-emerald-950' : 'text-white'}`}>{reportData.orderCount}</p>
                                </div>
                                <div className={`rounded-2xl p-4 border flex flex-col justify-center transition-all ${isLightTheme ? 'bg-white border-emerald-100' : 'bg-zinc-800/50 border-zinc-800'}`}>
                                    <p className={`text-[10px] uppercase font-bold tracking-wider mb-1 ${isLightTheme ? 'text-emerald-600/40' : 'text-zinc-500'}`}>Panier Moyen</p>
                                    <p className={`text-xl font-bold leading-none ${isLightTheme ? 'text-emerald-950' : 'text-white'}`}>{panierMoyen} €</p>
                                </div>
                                <div className={`rounded-2xl p-4 border flex flex-col justify-center transition-all ${isLightTheme ? 'bg-white border-emerald-100' : 'bg-zinc-800/50 border-zinc-800'}`}>
                                    <p className={`text-[10px] uppercase font-bold tracking-wider mb-1 ${isLightTheme ? 'text-emerald-600/40' : 'text-zinc-500'}`}>Top Vente</p>
                                    <p className={`text-sm font-bold truncate ${isLightTheme ? 'text-emerald-950' : 'text-white'}`} title={bestSeller ? bestSeller[0] : '-'}>
                                        {bestSeller ? bestSeller[0] : '-'}
                                    </p>
                                    {bestSeller && <p className={`text-xs font-bold mt-0.5 ${isLightTheme ? 'text-green-600' : 'text-green-400'}`}>{bestSeller[1].qty} vendus</p>}
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col h-full space-y-4">
                            ...
                            {reportMode === 'close' ? (
                                <div className={`rounded-2xl p-6 flex flex-col space-y-4 transition-all ${isLightTheme ? 'bg-white border border-emerald-100 shadow-sm' : 'bg-zinc-800/20 border border-zinc-800'}`}>
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                placeholder="0.00 €"
                                                className={`w-full text-2xl font-black rounded-2xl border px-6 py-4 focus:outline-none focus:ring-4 focus:ring-green-500/20 transition-all ${isLightTheme ? 'bg-white border-emerald-100 text-emerald-950 placeholder-emerald-100' : 'bg-black/60 border-zinc-700 text-white placeholder-zinc-800'}`}
                                            />
                                        </div>
                    ...
                    <div className={`p-6 border-t flex items-center justify-between transition-all ${isLightTheme ? 'bg-emerald-50/50 border-emerald-100' : 'bg-black/20 border-zinc-800'}`}>
                        <button
                            onClick={() => window.print()}
                            className={`flex items-center gap-2 px-4 py-2 font-bold text-sm rounded-xl transition-all ${isLightTheme ? 'text-emerald-600 hover:bg-emerald-50 border border-emerald-100' : 'text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700'}`}
                        >
                        ...
                        {reportMode === 'view' ? (
                            <button
                                onClick={onClose}
                                className={`bg-green-500 hover:bg-green-400 text-black font-bold py-3 rounded-2xl transition-all px-8 ${isLightTheme ? 'shadow-md shadow-green-200' : ''}`}
                            >
                                OK
                            </button>
                        ) : (
