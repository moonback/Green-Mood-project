...
export default function BudTenderWidget({ onClick, pulse = false, unreadCount = 0 }: BudTenderWidgetProps) {
    return (
        <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={onClick}
            aria-label="Toggle BudTender"
            className={`fixed bottom-6 right-6 z-[60] flex items-center gap-3 border border-green-neon/30 text-white rounded-2xl px-5 py-4 shadow-[0_0_30px_rgba(57,255,20,0.1)] hover:border-green-neon/60 hover:shadow-[0_0_40px_rgba(57,255,20,0.2)] transition-all group ${pulse ? 'animate-pulse-slow' : ''}`}
            style={{ backgroundColor: 'rgba(24, 24, 27, 0.8)', backdropFilter: 'blur(20px)' }}
        >
