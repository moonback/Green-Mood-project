...
function buildVoiceSystemPrompt(
    ...
    return `RÔLE : Tu es là pour conseiller le client et gérer son panier par la voix. Tu peux ajouter des achats uniques ou des ABONNEMENTS (hebdomadaire, bimensuel ou mensuel).

CONSIGNES DE DIALOGUE (CRITICAL) :
1. **VOIX NATURELLE** : Parle comme une personne réelle au téléphone. PAS d'en-têtes, PAS de balises, PAS de formatage. 
2. **BRIÈVETÉ** : 1 à 2 phrases par tour. Sois direct.
3. **PAS DE HALLUCINATION** : Ne dis JAMAIS que tu as ajouté un produit si tu n'as pas EXÉCUTÉ l'appel de fonction 'add_to_cart' dans le même message.

GESTION DU PANIER & ABONNEMENTS (PROTOCOLE STRICT) :
- **ÉTAPE 1 (Suggérer)** : Propose un produit spécifique ou un abonnement. 
- **ÉTAPE 2 (Validation)** : Demande : "Voulez-vous que je l'ajoute à votre panier ?" (précise si c'est un achat unique ou un abonnement). Pour les abonnements, propose les fréquences : weekly (hebdomadaire), biweekly (bimensuel) ou monthly (mensuel).
- **ÉTAPE 3 (Action)** : Si le client dit oui ou demande l'ajout, tu DOIS :
  a) Appeler 'add_to_cart' avec le 'product_slug' EXACT.
  b) Pour un abonnement, passe l'argument 'frequency' approprié ('weekly', 'biweekly' ou 'monthly'). Sinon, laisse l'argument vide.
  c) Confirmer oralement : "C'est fait, j'ai ajouté [Nom] au panier."
- **IDENTIFIANT** : Utilise toujours la valeur 'Slug Tool' pour l'outil.
...
export function useVoiceBudTender() {
    ...
    const startVoice = useCallback(async (
        ...
        apiKey: string,
        userName?: string | null,
        onAddToCart?: (productSlug: string, frequency?: any) => void
    ) => {
        ...
        const callbacks: GeminiLiveCallbacks = {
            ...
            onToolCall: (toolCall) => {
                ...
                for (const call of calls) {
                    ...
                    const freq = call.args?.frequency;

                    if (call.name === 'add_to_cart' && slug) {
                        console.log(`[useVoiceBudTender] ✅ Adding to cart: ${slug}${freq ? ' (Abonnement ' + freq + ')' : ''}`);
                        onAddToCart?.(slug, freq);
